...
def log_parameters_size(model):
    all_parameters_size = sum(p.numel() for p in model.parameters())
    lora_parameters_size = 0
    non_lora_parameters_size = 0
    for name, param in model.named_parameters():
        if param.requires_grad:
            if "lora" in name:
                lora_parameters_size += param.numel()
            else:
                non_lora_parameters_size += param.numel()
    wandb.log({
        "all_parameters_size": all_parameters_size,
